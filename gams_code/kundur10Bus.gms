$set matout "'matsol.gdx', model_status,solver_status,Vmag,Vr,Vi,Pg,Qg,Pl,Ql,Pflow,Qflow,Bsvc,sv,spg,sqg,spl,sql,sbsvc";
...
Parameter pf(LI),Pgmax(GI),Pgmin(GI),Qgmax(GI),Qgmin(GI),Vmax(BI),Vmin(BI),Plmin(LI),Plmax(LI),Qlmin(LI),Qlmax(LI),Bsvcmax(BI),Bsvcmin(BI),Bltc(LTCI,LTCI),Sgmax(GI);
$GDXIN mat2gms_varlim.gdx
$LOAD pf,Pgmax,Pgmin,Qgmax,Qgmin,Vmax,Vmin,Plmin,Plmax,Qlmin,Qlmax,Bsvcmax,Bsvcmin,Bltc,Sgmax
$GDXOUT

Parameter svmax(BI),svmin(BI),spgmax(GI),spgmin(GI),sqgmax(GI),sqgmin(GI),sbsvcmax(SVCI),sbsvcmin(SVCI),splmax(LI),splmin(LI),sqlmax(LI),sqlmin(LI);
$GDXIN mat2gms_slack.gdx
$LOAD svmax,svmin,spgmax,spgmin,sqgmax,sqgmin,splmax,splmin,sbsvcmax,sbsvcmin,sqlmax,sqlmin
$GDXOUT
...
Variables sv(BI),spg(GI),sqg(GI),spl(LI),sql(LI),sbsvc(SVCI);
************************************************************************************************************************************
************************************************************************************************************************************
* Assigning starting point

*execute_loadpoint 'OPF_trial_p.gdx';
Parameters Vmag_ini(BI),Pg_ini(GI),Qg_ini(GI),Pl_ini(LI),Ql_ini(LI),Vr_ini(BI),Vi_ini(BI),Bsvc_ini(SVCI);
$GDXIN mat2gms_ini.gdx
$LOAD Vmag_ini,Pg_ini,Qg_ini,Pl_ini,Ql_ini,Vr_ini,Vi_ini,Bsvc_ini
$GDXOUT
...
obj..                   z =e=  100*sum(GI,sqr(sv(GI)))  + 300*sum(GI,sqr(spg(GI)) + 100*sqr(sqg(GI))) + 1000*sum(LI,sqr(spl(LI))) + 500*sum(LI,sqr(sql(LI))) ;
...
ePbal(BI)..      sum(GI,busgenary(BI,GI)*Pg(GI)) - sum(LI,busloadary(BI,LI)*Pl(LI))
                        - sum(m,G(BI,m)*(Vr(BI)*Vr(m)+Vi(BI)*Vi(m)) + B(BI,m)*(-Vr(BI)*Vi(m)+Vi(BI)*Vr(m))) =e= 0;

eQbal(BI)..      sum(GI,busgenary(BI,GI)*Qg(GI)) - sum(LI,busloadary(BI,LI)*Ql(LI))
                 - sum(m,G(BI,m)*(-Vr(BI)*Vi(m)+Vi(BI)*Vr(m)) + B(BI,m)*(-Vr(BI)*Vr(m)-Vi(BI)*Vi(m))) =e= 0;
...
sl_Ql(LI)..              Ql(LI)   =e= Ql_ini(LI) + sql(LI);
