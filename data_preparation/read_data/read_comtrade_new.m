function read_comtrade_new(PathAndCfgName)
clc
%clear
...
% Type of data file
data_type = char(cfg_string{1,8+No_Ch}(1));
...
elseif (strcmpi(data_type,'float32'))
    ...
    NR1 = Ana_Ch;
    NR2 = ceil(Dig_Ch/16);
    fseek(dat_id,0,'bof'); % place at start
    dat(1:2,:) = fread(dat_id,[2,inf],'2*uint32',4*NR1+2*NR2,'ieee-le');
    l = size(dat,2);
    l = l-1;
    fseek(dat_id,8 , 'bof'); % place at first analog data
...
% All channels are extracted here, but the analogues still need scaling
 for i = 1 : No_Ch
    ...
    if ~isletter(var_string{i}(1))
        var_string{i} = strcat('x',var_string{1});
    end
