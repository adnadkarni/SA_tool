function [ db ] = read_pgcil10bus( pth )
...
for i=1:size(fileList,1)%[101,113,213]
    ...
    for j=1:size(Vmrs,1)
        ...
        if ~isempty(regexp(str,'VRVm'))
            ...
            split_strname = strsplit(str,'_');
            db.pmu_name{i,1} = strcat(split_strname{2},...
                '-',split_strname{5});
        ...
        elseif ~isempty(regexp(str,'VRVa'))
            [db.Va{i,1}(:,1)] = angle_correction_dbserver (eval(str));
        elseif ~isempty(regexp(str,'VYVa'))
            [db.Va{i,1}(:,2)] = angle_correction_dbserver (eval(str));
        ...
        elseif ~isempty(regexp(str,'IRVa'))
            [db.Ia{i,1}(:,1)] = angle_correction_dbserver (eval(str));
        elseif ~isempty(regexp(str,'IYVa'))
            [db.Ia{i,1}(:,2)] = angle_correction_dbserver (eval(str));
        ...
        elseif ~isempty(regexp(str,'dtdf'))
